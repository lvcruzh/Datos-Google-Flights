...
sem <- "1"
dia <- "14-03-19"
data <- readr::read_csv(paste0("D:/WebScraping-GoogleFlights/",dia,"/Resultados_",sem,"sem2.csv"))
data$X1 <- NULL
names(data) <- c("x")
...
semana <- as.integer(sem)
tab <- cbind(aerolinea,vuelo,hora,precio,duracion,escalas,fecha,semana)
names(tab) <- c("aerolinea","vuelo","horario","precio","duracion","escalas","fecha","semana-anticipada")
tab <- as.data.frame(tab)

#7.Guardar el archivo
write.csv(tab,paste0("D:/WebScraping-GoogleFlights/",dia,"/Base",sem,"sem.csv"), fileEncoding = "UTF-8")
